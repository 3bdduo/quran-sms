        ...
        <div className="mt-12 sm:mt-16 space-y-6 sm:space-y-8">
          {tracks.map(({ icon: Icon, title, duration, ages, points }, i) => (
            <Reveal key={title} from={i % 2 === 1 ? "end" : "start"} distance={50}>
              ...
              >
                <div className="shrink-0 h-24 w-24 rounded-3xl bg-brand-soft group-hover:bg-brand flex items-center justify-center text-brand-ink group-hover:text-on-brand transition-all duration-300 sh-soft group-hover:rotate-[-6deg]">
                  <Icon size={44} />
                </div>
