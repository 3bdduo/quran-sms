...
export function CurriculumPreview() {
  return (
    <section className="py-16 sm:py-24 bg-deep text-on-deep relative overflow-hidden shadow-[0_-24px_60px_-30px_rgba(0,0,0,0.5),0_24px_60px_-30px_rgba(0,0,0,0.5)]">
      <div aria-hidden="true" className="absolute inset-0 pattern-star opacity-[0.07]" />
      <div aria-hidden="true" className="absolute -top-32 left-[10%] h-80 w-80 rounded-full bg-glow-2 blur-3xl animate-float-slow" />
      <div aria-hidden="true" className="absolute -bottom-32 right-[8%] h-80 w-80 rounded-full bg-glow-1 blur-3xl animate-float" />
        ...
        <Stagger className="mt-12 sm:mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {tracks.map(({ icon: Icon, title, desc }) => (
            <StaggerItem key={title} className="h-full">
              <div className="group h-full bg-deep-2/70 backdrop-blur-sm border border-deep-line rounded-3xl p-6 sm:p-7 shadow-[0_18px_40px_-16px_rgba(0,0,0,0.6)] transition-all duration-500 hover:-translate-y-2 hover:bg-deep-2 hover:border-gold/50 hover:shadow-[0_30px_60px_-18px_rgba(0,0,0,0.75)]">
                <div className="mb-5 h-14 w-14 grid place-items-center rounded-2xl bg-gold/15 text-gold transition-transform duration-500 group-hover:scale-110 group-hover:rotate-[-6deg]">
                  <Icon size={28} />
                </div>
