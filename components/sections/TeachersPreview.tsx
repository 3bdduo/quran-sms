          ...
          <div className="mt-12 sm:mt-16 grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-10 sm:gap-8">
            {featured.map((t, i) => (
              <Reveal key={t.id} delay={i * 0.09} className="text-center group">
                <div className="relative mx-auto h-28 w-28 sm:h-36 sm:w-36 lg:h-40 lg:w-40 rounded-full p-1.5 bg-linear-to-br from-gold via-brand to-brand-strong sh-lift transition-transform duration-300 group-hover:scale-105 group-hover:-rotate-3">
                  <div className="relative h-full w-full rounded-full overflow-hidden bg-brand-soft ring-4 ring-surface">
                    {t.photo_url ? (
                      <Image src={t.photo_url} alt={t.name} fill className="object-cover transition-transform duration-400 group-hover:scale-110" sizes="(min-width: 1024px) 160px, (min-width: 640px) 144px, 112px" />
                    ) : (
